...
class AirspaceVisualizer(QMainWindow):
    ...
    def __init__(self):
        ...
        # MacBook Air M3 13" için pencere boyutunu ayarla
        # Standart çözünürlük 2560x1664, ekranın %80'i kadar bir alan kullanalım
        window_width = 2050  # 2560 * 0.8
        window_height = 1330  # 1664 * 0.8
        ...
        # Create left sidebar container with adjustable width
        self.left_sidebar_container = QWidget()
        self.left_sidebar_container.setMinimumWidth(200)
        ...
        try:
            ...
            self.splitter.setSizes([250, 1350]) # Fallback sizes
    ...
    def create_menu_actions(self):
        ...
        # Add Gradient Calculator action
    ...
    def connect_signals(self):
        ...
        # Çizgi kalınlığı ve renk ayarları için sinyal bağlantıları
        self.left_sidebar.routeLineWidthChanged.connect(self.on_route_line_width_changed)
        self.left_sidebar.selectedRouteLineWidthChanged.connect(self.on_selected_route_line_width_changed)
        self.left_sidebar.routeColorChanged.connect(self.on_route_color_changed)
        self.left_sidebar.selectedRouteColorChanged.connect(self.on_selected_route_color_changed)
        ...
        self.left_sidebar.snapGridSpacingChanged.connect(self.on_snap_grid_spacing_changed)
    ...
    def initialize_ui_settings(self):
        ...
        # Çizgi kalınlıkları için slider değerlerini ayarla
        self.left_sidebar.route_line_width_slider.setValue(self.map_widget.normal_route_line_width)
        self.left_sidebar.selected_route_line_width_slider.setValue(self.map_widget.selected_route_line_width)
        
        # Renk butonlarını ayarla
        self.left_sidebar.update_route_color_button(self.map_widget.route_default_color)
        self.left_sidebar.update_selected_route_color_button(self.map_widget.route_selected_color)
    ...
    def toggle_left_sidebar(self):
        ...
        if self.left_sidebar_container.isVisible():
            ...
            # Restore left sidebar width
            sizes = self.splitter.sizes()
            self.splitter.setSizes([250, sizes[1] - 250])
    ...
    def toggle_route_drawing(self, checked):
        ...
        if checked:
            ...
            # Gradient Calculator'ı aç
            if not hasattr(self, 'gradient_calculator') or not self.gradient_calculator.isVisible():
                self.show_gradient_calculator()
        else:
    ...
    def show_gradient_calculator(self):
        ...
        # Get waypoints from the map widget
        waypoints = []
        for route in self.map_widget.drawn_elements['routes']:
            if 'points' in route:
                waypoints.extend([(p[0], p[1]) for p in route['points']])
        
        # Waypoint olmasa bile hesaplayıcı diyalogunu aç
        # Create and show the gradient calculator dialog
        self.gradient_calculator = GradientCalculatorDialog(waypoints, self)
    ...
    def update_gradient_calculator(self, *args):
        ...
        if hasattr(self, 'gradient_calculator') and self.gradient_calculator.isVisible():
            ...
            # Update gradient calculator
            self.gradient_calculator.set_waypoints(waypoints)
            
            # Recalculate if there are waypoints (otomatik olarak hesapla)
            if len(waypoints) >= 2:
                self.gradient_calculator.calculate_gradient()

    def on_route_point_added(self, route_points):
        """Her yeni waypoint eklendiğinde çağrılır"""
        # Gradient Calculator'ı güncelle veya göster
        if hasattr(self, 'gradient_calculator') and self.gradient_calculator.isVisible():
            # Waypoints listesini gradient calculator için formata çevir
            waypoints = [(lat, lon) for lat, lon in route_points]
            
            # Gradient calculator'ı güncelle
            self.gradient_calculator.set_waypoints(waypoints)
            
            # Yeterli waypoint varsa hesaplamayı otomatik yap
            if len(waypoints) >= 2:
                self.gradient_calculator.calculate_gradient()
    ...
    def on_snap_grid_spacing_changed(self, value):
        """Snap ızgara aralığını değiştir"""
        if hasattr(self.map_widget, 'route_drawer') and hasattr(self.map_widget.route_drawer, 'snap_manager'):
            self.map_widget.route_drawer.snap_manager.set_snap_grid_spacing(value)
            self.map_widget.update()
            self.map_widget.update_status_message(f"Snap ızgara aralığı: {value} NM")

    def on_route_line_width_changed(self, width):
        """Rota çizgisi kalınlık ayarının değiştiğinde tetiklenen metot"""
        self.map_widget.normal_route_line_width = width
        self.map_widget.update()  # Haritayı güncelle
        self.statusBar().showMessage(f"Rota çizgi kalınlığı: {width}px", 2000)
    
    def on_selected_route_line_width_changed(self, width):
        """Seçili rota çizgisi kalınlık ayarının değiştiğinde tetiklenen metot"""
        self.map_widget.selected_route_line_width = width
        self.map_widget.update()  # Haritayı güncelle
        self.statusBar().showMessage(f"Seçili rota çizgi kalınlığı: {width}px", 2000)
    
    def on_route_color_changed(self, color):
        """Rota rengi değiştiğinde tetiklenen metot"""
        if isinstance(color, QColor) and color.isValid():
            self.map_widget.route_default_color = color
            self.map_widget.colors['route_default'] = color
            self.map_widget.update()  # Haritayı güncelle
            self.statusBar().showMessage(f"Rota rengi değiştirildi: {color.name()}", 2000)
    
    def on_selected_route_color_changed(self, color):
        """Seçili rota rengi değiştiğinde tetiklenen metot"""
        if isinstance(color, QColor) and color.isValid():
            self.map_widget.route_selected_color = color
            self.map_widget.colors['route_selected'] = color
            self.map_widget.update()  # Haritayı güncelle
            self.statusBar().showMessage(f"Seçili rota rengi değiştirildi: {color.name()}", 2000)
