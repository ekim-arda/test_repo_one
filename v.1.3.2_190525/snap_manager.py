...
class SnapManager(QObject):
    ...
    SNAP_ALL = 31  # Tüm modlar aktif (31 = 1+2+4+8+16)
    ...
    def __init__(self, map_widget):
        ...
        self.snap_grid_spacing = 5.0  # NM cinsinden ızgara aralığı
    ...
    def set_snap_grid_spacing(self, spacing):
        """Izgara aralığını deniz mili cinsinden ayarla"""
        self.snap_grid_spacing = max(0.1, min(50.0, spacing))  # 0.1-50 NM aralığında sınırla
    ...
    def find_snap_points(self, mouse_pos):
        ...
        # Izgara için snap noktaları bul
        if self.snap_mode & self.SNAP_GRID:
            self._find_grid_snap_points(mouse_pos)
    ...
    def _find_grid_snap_points(self, mouse_pos):
        """Koordinat ızgarasındaki snap noktalarını bul"""
        # Izgara çizgileri aralığı (NM)
        grid_spacing = self.snap_grid_spacing  # NM
        
        # Fare pozisyonu için coğrafi koordinatları al
        lat, lon = self.map_widget.screen_to_geo(mouse_pos.x(), mouse_pos.y())
        
        # En yakın ızgara noktalarını hesapla (enlem/boylam bazında)
        # Enlemdeki 1 derece yaklaşık 60 NM'dir
        lat_spacing = grid_spacing / 60.0
        # Boylamdaki derece mesafesi enleme bağlı olarak değişir (cos(lat) faktörü)
        lon_spacing = grid_spacing / (60.0 * math.cos(math.radians(lat)))
        
        # En yakın ızgara noktalarını hesapla
        lat_grid = round(lat / lat_spacing) * lat_spacing
        lon_grid = round(lon / lon_spacing) * lon_spacing
        
        # Izgara noktasının ekran koordinatlarını hesapla
        grid_screen = self.map_widget.geo_to_screen(lat_grid, lon_grid)
        
        # Yeterince yakınsa, snap noktası olarak ekle
        if (grid_screen - mouse_pos).manhattanLength() <= self.snap_tolerance:
            desc = f"Izgara: {lat_grid:.6f}, {lon_grid:.6f}"
            self.snap_points.append(SnapPoint(grid_screen, (lat_grid, lon_grid), desc, "grid"))
