...
class MapWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.waypoint_size = 5  # Waypoint boyutu (piksel)
        self.route_id_counter = 0 # Add counter for unique route IDs
        
        # Çizgi kalınlığı ayarları
        self.normal_route_line_width = 2     # Normal rota çizgi kalınlığı (px)
        self.selected_route_line_width = 4   # Seçili rota çizgi kalınlığı (px)
        
        # TMA sınır ayarları
        self.show_tma_boundary = True
        self.tma_boundary_color = QColor(255, 68, 0)  # Turuncu renk
        self.tma_boundary_width = 1.5  # Çizgi kalınlığı
        ...
        if event.modifiers() & Qt.ControlModifier:
            if self.selected_path_index is not None and \
               self.drawn_elements['routes'][self.selected_path_index].get('type') == 'pointmerge':
                ...
                route['track_angles'] = self.calculate_track_angles(new_points)
                ...
                if route.get('type') == 'user_route':
                    for wp_index, (lat, lon) in enumerate(route['points']):
                        ...
                        if (screen_pos - pos).manhattanLength() < self.WAYPOINT_SELECTION_TOLERANCE:
                            ...
                            # Update segment distances and track angles
                            route['segment_distances'] = self.calculate_segment_distances(route['points'])
                            route['track_angles'] = self.calculate_track_angles(route['points'])
    ...
    def paintEvent(self, event):
        ...
        for i, route in enumerate(self.drawn_elements['routes']):
            if 'points' in route:
                ...
                # Segment mesafelerini çizgilerin üzerine yaz - göster/gizle seçeneğine bağlı olarak
                if self.show_segment_distances and 'segment_distances' in route and len(route['segment_distances']) > 0:
                    for i in range(len(route['points']) - 1):
                        ...
                        # Mesafeyi formatla (1 ondalık basamak)
                        distance_text = f"{route['segment_distances'][i]:.1f} NM"
                        ...
                        # Yeni font ile metin boyutunu tekrar hesapla
                        text_rect = painter.fontMetrics().boundingRect(distance_text)
                        bg_rect = QRectF(mid_x - text_rect.width()/2 - 2, 
                                         mid_y - text_rect.height()/2 - 2,
                                         text_rect.width() + 4, 
                                         text_rect.height() + 4)
                        
                        # Beyaz yarı-saydam arka plan (daha opak)
                        painter.setBrush(QBrush(QColor(255, 255, 255, 230)))
                        painter.setPen(Qt.NoPen)
                        painter.drawRect(bg_rect)
                        
                        # Mesafe metnini koyu renkte yaz
                        painter.setPen(QPen(QColor(0, 0, 0), 1))
                        painter.drawText(QPointF(mid_x - text_rect.width()/2, mid_y + text_rect.height()/3), distance_text)
                ...
                if pattern_type == 'pointmerge' and i == self.selected_path_index:
                    ...
                    if len(route['points']) >= 2:
                        ...
                        painter.setBrush(Qt.NoBrush)
                        dash_pen = QPen(QColor(255, 165, 0), 1.5, Qt.DashLine)  # Orange dashed line, thinner
                        dash_pen.setDashPattern([50, 50])  # Dash pattern: 5px dash, 5px space
                        painter.setPen(dash_pen)
    ...
    def draw_path_extension(self, config, route_id_to_update=None):
        ...
        if route_id_to_update:
            ...
            if waypoints_data and len(waypoints_data) > 0:
                ...
                # Update segment distances and track angles in the config (important for sidebar display)
                updated_route_config['segment_distances'] = self.calculate_segment_distances(points_tuples)
                updated_route_config['track_angles'] = self.calculate_track_angles(points_tuples)
    ...
    # contextMenuEvent metodu kaldırıldı - sağ tıklama menüsü işlevi artık kullanılmamaktadır